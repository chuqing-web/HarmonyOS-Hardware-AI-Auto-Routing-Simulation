...
export class PinWorldResolver {
    ...
    static forDeviceInst(dev: DeviceInst, pinId: string, pinName: string): Point2D {
        return PinWorldResolver.forDevice(dev.libDevId, dev.x, dev.y, dev.rotate, dev.mirrorH, pinId, pinName);
    }
    static forComponent(comp: ComponentInstance, pinId: string, pinName: string): Point2D {
        return PinWorldResolver.forDevice(comp.libraryId, comp.position.x, comp.position.y, comp.rotation, comp.mirrored, pinId, pinName);
    }
}
