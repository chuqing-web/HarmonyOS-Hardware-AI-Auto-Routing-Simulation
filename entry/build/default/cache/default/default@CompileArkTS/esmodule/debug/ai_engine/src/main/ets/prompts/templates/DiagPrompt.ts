...
export const DIAG_PROMPT: PromptTemplate = {
    id: 'diag_v5',
    version: '5.0.0',
...
3. 导线几何 → 是否侵入选中命中区(HIT_PAD=22)？是否贴近无关引脚(≥20mil)？
4. 仿真波形 → 动态行为异常
5. 器件参数 → 设计值合理性
...
- 功率表 V 并 / I 串；禁止 I 与 V 同节点对
- 万用表 VIRTUAL_METER 真脚 V,A,OHM,COM（勿写 V+）
- 所有仪器关键引脚是否已连接（AM I+/I-；VM V+/COM；DMM 按档位；OSC CH1+GND；LA CH1+GND；UART TX/RX/GND；FC IN/GND；SG OUT/GND）
- 一般电路未用示波器 CH2–4 悬空可接受；教学全套仪器应接满；禁止误接到 GND/NC
