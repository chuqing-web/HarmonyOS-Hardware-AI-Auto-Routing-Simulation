...
export const PCB_ROUTE_PROMPT: PromptTemplate = {
    id: 'pcb_route_v1',
    version: '1.2.0',
    system: `你是 PCB 层角色与布线约束填写器。必须为当前板的每一个铜层指定 layerRoles，禁止省略，禁止输出走线坐标。
...
【JSON】
{"layerRoles":{"F.Cu":"stub","B.Cu":"signal_h"},"netPriority":{"GND":100,"VCC":90,"VOUT":50},"viaPreference":{"kind":"through","preferThrough":true},"globalConstraint":""}

layerRoles 的 key 必须与铜层列表完全一致。`,
    userTemplate: '铜层列表：{{copper_layers}}\n网络策略摘要：{{net_plan_summary}}\n板摘要：{{board_summary}}\n\n现在立即只输出 JSON，不要任何其它文字：'
};
