import { IdUtil, makeRouteLine, Logger, INSTR_TRACE_TAG, traceAiDiag, DeviceHitGeometry } from "@bundle:com.elecdraw.aischsim/entry@common/Index";
import type { SchTopology, NetInfo, NetLabelInfo, NetNodeRef, DeviceInst, WorldHitRect } from "@bundle:com.elecdraw.aischsim/entry@common/Index";
...
export class AiTopologyFixKit {
    ...
    /**
     * 根据 auditCriticalElectrical 结果做确定性拆短路。
     */
    static healCriticalElectricalShorts(topo: SchTopology, wiring: ConstrainedWiringEngine): TopologyFixResult {
        ...
        for (let di = 0; di < topo.deviceList.length; di++) {
            ...
            if (lib.indexOf('AMMETER') >= 0) {
                // I+ → VCC, I- → VCC_AM（串联中间点）
                const r1 = AiTopologyFixKit.reconnectPin(topo, wiring, d.refName, 'I+', 'VCC');
                const r2 = AiTopologyFixKit.reconnectPin(topo, wiring, d.refName, 'I-', 'VCC_AM');
        ...
        if (fixed > 0) {
            Logger.info(INSTR_TRACE_TAG, `[AI_FIX] healCriticalElectricalShorts fixed=${fixed} errsWas=${errs.length}`);
        }
        return { fixed, needReroute: fixed > 0, notes };
    }
    /**
    ...
    static wireOpAmpFeedback(topo: SchTopology, wiring: ConstrainedWiringEngine): TopologyFixResult {
        ...
        for (let i = 0; i < topo.deviceList.length; i++) {
            ...
            for (let ci = 0; ci < channels.length; ci++) {
                ...
                // 双输入已接且不同网 → 有意比较器，不改成跟随
                if (innNet.length > 0 && inpNet.length > 0 &&
                    innNet.toUpperCase() !== inpNet.toUpperCase()) {
                    continue;
                }
    ...
    static finalizeForGate(topo: SchTopology, wiring: ConstrainedWiringEngine, opts?: FinalizeGateOpts): TopologyFixResult {
        ...
        fixed += prune.fixed;
        // 二次剪：任意仅 1 节点且非电源的网（开路 ERC）
        const prune2 = AiTopologyFixKit.pruneAllSingletonSignalNets(topo);
        fixed += prune2.fixed;
        if (fixed > 0) {
    ...
    /**
     * 修剪所有单脚信号网（保留 VCC/GND/VDD/VSS），消除开路 ERC。
     * 单脚电源网不删（符号自身）。
     */
    static pruneAllSingletonSignalNets(topo: SchTopology): TopologyFixResult {
        const notes: string[] = [];
        let fixed = 0;
        ...
        Logger.info(INSTR_TRACE_TAG, `[AI_FIX] pruneAllSingletonSignalNets fixed=${fixed} | ${notes.slice(0, 8).join('; ')}`);
        return { fixed, needReroute: false, notes };
    }
    /**
        ...
        for (let ni = 0; ni < topo.netList.length; ni++) {
            ...
            for (let ki = 0; ki < net.nodeList.length; ki++) {
                ...
                const isSense = /^CH\d+$/.test(pinU) || pinU === 'V+' || pinU === 'V' ||
                    pinU === 'IN' || pinU === 'I+' || pinU === 'I-' || pinU === 'PROBE';
                // 仪器 GND/COM 应在 GND 网
                if (pinU === 'GND' || pinU === 'COM') {
    ...
    private static heuristicNetForPin(dev: DeviceInst, pinId: string, topo: SchTopology): string {
        ...
        // R/C 串联 RC：若存在电容，R.2 与 C.1 共用 RC_MID
        const hasCap = topo.deviceList.some(d => (d.libDevId ?? '').startsWith('C_'));
        const hasRes = topo.deviceList.some(d => (d.libDevId ?? '').startsWith('R_'));
        ...
        // 仪器：I+/I- 必须异网；仅 CH1/V+ → 信号；未用 CH2+ 保持悬空
        if (AiTopologyFixKit.isInstrumentLib(lib)) {
            ...
            if (pinU === 'CH1' || pinU === 'V+' || pinU === 'IN' || pinU === 'PROBE') {
    ...
    private static addLabelStubForPin(topo: SchTopology, wiring: ConstrainedWiringEngine, net: NetInfo, dev: DeviceInst, pinId: string, pinName: string): void {
        const pinPos = PinWorldResolver.forDeviceInst(dev, pinId, pinName);
        // 粗算 hit：用 SELECTION_HIT_PAD 扩一圈
        ...
        // 尽量用引擎 demote 单网（会重建该网全部 stub，副作用可接受）
        ...
        const labelPos = DeviceHitGeometry.stubLabelOutsidePinAvoidForeign(pinPos, hit, [], 20);
