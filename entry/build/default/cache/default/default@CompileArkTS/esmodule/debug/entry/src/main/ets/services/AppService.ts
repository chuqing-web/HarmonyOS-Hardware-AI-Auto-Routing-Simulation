        ...
        try {
            ...
            if (!result.success || !result.topology) {
                ...
                if (incompleteGate && result.topology) {
                    ...
                    this.appendAiGenLog('assistant', `生图未完成（ERC/几何门禁）：${result.errMsg || '仍有阻断项'}`);
                    if (mode === 'replace') {
                    ...
                    this.appendAiGenLog('system', `已落图供检视，但生图过程未完成：仍有 ${errN} 条 ERC 阻断项（含功能影响 warning）。` +
                        `请勿以此图仿真；请自检修复或重新生成。`);
                    this.onProjectChanged();
                    this.endAiGenerate(false);
                    this.onStatusMessage(result.errMsg || '生图未完成：仍有 ERC/几何阻断项');
                    return false;
                }
            ...
            this.appendAiGenLog('assistant', `落图完成 · 模式=${mode === 'replace' ? '替换' : '追加'} · ERC 阻断 ${errN} / 软警告 ${warnN}` +
                ` · 悬空器件 ${floatingComps}`);
            // 落图后二次门禁：error/critical + 功能影响 warning 均须清零
            if (errN > 0) {
                const issues = this.evaluatePostGenerateIssues(erc);
                this.aiSelfCheckPromptPending = true;
                this.appendAiGenLog('system', `生图未完成：落图后仍有 ${errN} 条阻断项（${issues.summary}）。` +
                    `功能影响 warning 亦不可放行。请「AI 自检修复」或重新生成；本次不算完整过程。`);
                this.onAiSelfCheckNeeded(issues.count, issues.summary);
                Logger.error(INSTR_TRACE_TAG, `[AI_GEN] INCOMPLETE post-load ercBlocking=${errN}`);
                traceAiOp('AI_GEN', 'generate_incomplete_post_erc', `ercBlocking=${errN}`);
                this.onProjectChanged();
                this.endAiGenerate(false);
                this.onStatusMessage(`生图未完成：仍有 ${errN} 条 ERC 阻断项`);
                return false;
            }
            const issues = this.evaluatePostGenerateIssues(erc);
            if (issues.needAsk) {
            ...
            this.appendAiGenLog('system', '画布已解锁');
            this.onProjectChanged();
            this.onStatusMessage(`AI 闭环完成: ${result.topology.deviceList.length} 器件, ${result.topology.wireList.length} 导线 · ERC 清零`);
            this.endAiGenerate(true);
            return true;
        }
        catch (e) {
