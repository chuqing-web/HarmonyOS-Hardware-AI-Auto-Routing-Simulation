...
import type { ErcError, RouteLine } from '../types/TopologyTypes';
...
export function traceCaptureWave(channel: number, probe: string, source: string, pointCount: number, lastV: number, vpp: number = 0, rawSpanSec: number = 0, estFreq: number = 0, timebaseLabel: string = ''): void {
    const spanMs = rawSpanSec * 1e3;
    const tb = timebaseLabel.length > 0 ? ` tb=${timebaseLabel}` : '';
    const msg = `scope CH${channel + 1} probe=${probe.length > 18 ? probe.substring(0, 18) : probe} ` +
        ...
        `Vpp=${vpp.toFixed(4)}V span=${spanMs.toFixed(3)}ms f=${estFreq > 0 ? estFreq.toFixed(1) : '--'}Hz${tb}`;
    // 短窗/低 Vpp / 长窗(易左侧拥挤) 用 info，便于现场确认
    if (vpp < 0.05 || rawSpanSec < 5e-4 || rawSpanSec > 0.5 ||
        source === 'history' || source === 'flatDC' || source === 'historyLonger') {
        Logger.info(INSTR_TRACE_TAG, msg);
    }
    else {
        Logger.debug(INSTR_TRACE_TAG, msg);
...
/**
 * AI 画线明细 — 过滤 instr_trace 前缀 [AI_WIRE] DRAW
 * via: war | net_plan_stub | fix_stub | a_star | other
 */
export function traceAiWireDraw(via: string, detail: string): void {
    Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] DRAW via=${via} | ${detail}`);
}
/**
 * AI 拆线/改线明细 — 过滤 [AI_WIRE] DROP / MOVE
 * action: DROP（删导线）| MOVE（脚改挂网）| KEEP（幂等跳过）
 */
export function traceAiWireFix(action: string, detail: string): void {
    Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] ${action} | ${detail}`);
}
/**
 * AI 导线清单快照 — 过滤 [AI_WIRE] INV
 * stage 例：net_plan_end / war_end / fix_batch_end / finalize
 */
export function traceAiWireInventory(stage: string, wires: RouteLine[], netNameOf: (netUuid: string) => string, maxLines: number = 40): void {
    const n = wires.length;
    const show = Math.min(n, Math.max(1, maxLines));
    Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] INV ${stage} | total=${n}` +
        `${n > show ? ` show=${show}` : ''}`);
    if (n === 0) {
        Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] INV ${stage} | (no wires)`);
        return;
    }
    for (let i = 0; i < show; i++) {
        const w = wires[i];
        const pts = w.points ?? [];
        const a = pts.length > 0 ? pts[0] : undefined;
        const b = pts.length > 0 ? pts[pts.length - 1] : undefined;
        const ends = a && b
            ? `(${Math.round(a.x)},${Math.round(a.y)})→(${Math.round(b.x)},${Math.round(b.y)})`
            : '(no-pts)';
        Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] INV|${stage}|${i} wireId=${w.uuid ?? '?'} net=${netNameOf(w.netUuid)}` +
            ` pts=${pts.length} ${ends}`);
    }
    if (n > show) {
        Logger.info(INSTR_TRACE_TAG, `[AI_WIRE] INV ${stage} <<< truncated +${n - show}`);
    }
}
...
/**
 * 落线端点吸附决策：脚优先 vs 导线 T 接。
 * 排查「看起来连到仪器脚，实际被吸到附近铜皮」根因。
 */
export function traceWireSnapEndpointDecision(endLabel: string, rawX: number, rawY: number, resultX: number, resultY: number, mode: string, detail: string): void {
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] SNAP_EP ${endLabel} raw=(${Math.round(rawX)},${Math.round(rawY)}) ` +
        `→ (${Math.round(resultX)},${Math.round(resultY)}) mode=${mode} ${detail}`);
}
/** 导线↔导线并网（共端点 / T 接） */
export function traceWireWireMerge(kind: string, wireA: string, netA: string, wireB: string, netB: string, atX: number, atY: number, canonicalNet: string): void {
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] WIRE_WIRE ${kind} ` +
        `A=${wireA}(${netA}) B=${wireB}(${netB}) ` +
        `at=(${Math.round(atX)},${Math.round(atY)}) → net=${canonicalNet}`);
}
/** 导线↔引脚挂网（端点吸附 / 中段压脚 / 共位并脚） */
export function traceWirePinAttach(kind: string, refDes: string, pinName: string, pinId: string, wireId: string, netId: string, dist: number, worldX: number, worldY: number): void {
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] WIRE_PIN ${kind} ${refDes}.${pinName}(${pinId}) ` +
        `wire=${wireId} net=${netId} dist=${dist.toFixed(2)} ` +
        `at=(${Math.round(worldX)},${Math.round(worldY)})`);
}
/** 引脚↔引脚共位并网（异器件压在同一点） */
export function tracePinPinMerge(refA: string, pinA: string, refB: string, pinB: string, netId: string, atX: number, atY: number): void {
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] PIN_PIN coloc ${refA}.${pinA} + ${refB}.${pinB} ` +
        `→ net=${netId} at=(${Math.round(atX)},${Math.round(atY)})`);
}
/** 移动器件时导线是否跟上引脚 */
export function traceCompMoveWireFollow(refDes: string, compId: string, pinHits: number, wireUpdates: number, midSegSplits: number, misses: string): void {
    const missPart = misses.length > 0 ? ` miss=[${misses}]` : '';
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] COMP_MOVE ${refDes}(${compId}) pinHits=${pinHits} ` +
        `wiresUpdated=${wireUpdates} midSegSplit=${midSegSplits}${missPart}`);
}
/** 端点被纠正到精确引脚坐标（防移动后脱线） */
export function traceWireEndpointNudge(wireId: string, endLabel: string, fromX: number, fromY: number, toX: number, toY: number, refDes: string, pinName: string): void {
    Logger.info(INSTR_TRACE_TAG, `[WIRE_CONN] NUDGE ${endLabel} wire=${wireId} ` +
        `(${Math.round(fromX)},${Math.round(fromY)})→(${Math.round(toX)},${Math.round(toY)}) ` +
        `→ ${refDes}.${pinName}`);
}
