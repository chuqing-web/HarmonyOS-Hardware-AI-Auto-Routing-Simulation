...
【输出铁律 — 全阶段统一强制】:
1. 只输出一个 JSON 对象：第一字符必须是 { ，最后字符必须是 }
2. 禁止 JSON 外任何文字：说明、推理、电路分析、引脚讨论、markdown、代码围栏、复述规则
3. 可在模型内部思考，但思考内容不得出现在回复正文
4. 字段名可用 camelCase 或 snake_case；不要输出 JSON 以外的任何内容`;
