...
export class HardwareFingerprint {
    static collect(): string {
        ...
        let udid = 'unknown';
        try {
            serial = HardwareFingerprint.safe(deviceInfo.serial);
        }
        catch (_e) { /* no perm */ }
        try {
            udid = HardwareFingerprint.safe(deviceInfo.udid);
        }
        catch (_e) { /* no perm */ }
        const parts: string[] = [
