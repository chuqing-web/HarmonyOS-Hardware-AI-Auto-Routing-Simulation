    ...
    static analyze(waves: WaveData[], digitalStates: Map<string, string>): ErcViolation[] {
        ...
        for (let w = 0; w < waves.length; w++) {
            ...
            if (wave.timeAxis.length >= 4) {
                ...
                if (freq > 0 && freq < 10) {
                    violations.push(DynamicErcEngine.violation(ErcSeverity.WARNING, `频率异常偏低: ${wave.probeName} ≈ ${freq.toFixed(1)}Hz`, wave.netName, '检查时钟源或 RC 时间常数'));
                }
                if (freq > 50e6) {
                    violations.push(DynamicErcEngine.violation(ErcSeverity.ERROR, `频率异常偏高: ${wave.probeName} ≈ ${(freq / 1e6).toFixed(1)}MHz`, wave.netName, '检查信号完整性或 EMI'));
                }
            }
            if (wave.waveType === 'current' || wave.currentAxis.length > 0) {
                ...
                if (iMax > 0.5) {
                    violations.push(DynamicErcEngine.violation(ErcSeverity.ERROR, `过流检测: ${wave.probeName} 峰值 ${iMax.toFixed(3)}A`, wave.netName, '检查负载短路或限流电阻'));
                }
            }
        }
        digitalStates.forEach((state: string, pinId: string) => {
            if (state === 'X' || state === 'conflict') {
                violations.push(DynamicErcEngine.violation(ErcSeverity.ERROR, `数字总线冲突/高阻异常: ${pinId}`, pinId, '检查输出引脚短接'));
            }
        });
        return violations;
    }
