...
2. 拆成 2～4 个功能模块（modules）。每个模块必须有:
   ...
   - boundaryPins: 本模块对外暴露的引脚，格式仅 "RefDes.Pin"（不要写 ModuleId 前缀），至少 1 个
3. joints: 模块间 pin↔pin 清单（合并时按此用【同名网络标号】并网，不拉跨模块长线）
   - from / to 格式: "{ModuleId}.{RefDes}.{Pin}" 例如 "M1.RV1.W"、"M1.U1.3"
   - 【对照】boundary 写 "R1.1"，joints 对应端写 "M1.R1.1"（多 ModuleId 前缀）
...
6. 禁止 markdown；只输出纯 JSON；第一字符 { 最后字符 }
...
Schema:
{
  ...
  "joints":[
    ...
    userTemplate: `用户需求:\n{{user_prompt}}\n\n现在立即只输出整体设计 JSON（modules+boundaryPins+joints；boundaryPin 都进 joints；POWER 进 joints；型号须库内 libDevId；位号≠型号；真脚；勿擅自加芯片/仪器），不要任何其它文字：`
};
