...
export function buildLabMcu8051(doc) {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const { c1, c2 } = placeXtalCaps(doc, ox + 40, 310, `CX${i}`);
 ...
 */
export function buildLabDigitalGates(doc) {
  const vcc = K.place(doc, 'VCC', 'PWR1', { x: 40, y: 40 });
  const gnd = K.place(doc, 'GND', 'GND1', { x: 40, y: 520 });
 ...
 */
export function buildLabSchmitt(doc) {
 ...
 */
export function buildLabIntegrator(doc) {
  ...
  const gnd = K.place(doc, 'GND', 'GND1', { x: 60, y: 360 });
  const sig = K.place(doc, 'SIGNAL_GEN', 'SG1', { x: 60, y: 180 });
  sig.parameters.waveform = 'square';
  sig.parameters.amplitude = '5V';
  sig.parameters.frequency = '1kHz';
  sig.parameters.offset = '0V';
  sig.parameters.dutyCycle = '50%';
 ...
 */
export function buildLab555Astable(doc) {
  const vcc = K.place(doc, 'VCC', 'PWR1', { x: 40, y: 40 });
  vcc.parameters.voltage = '5V';
...
export const TEMPLATE_DEFS = [
  ...
  { id: 'lab_passive', name: '无源器件检测', description: '全部电阻/电容/电感/电位器/LC/交流源', build: buildLabPassive },
  ...
  { id: 'lab_digital_gates', name: '数字门电路实验',
    description: 'Digital IC×7：按键真值表(六门LED) + 10Hz 时钟驱 CD4017',
