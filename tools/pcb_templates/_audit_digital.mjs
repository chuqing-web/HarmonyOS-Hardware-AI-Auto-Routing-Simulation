...
const path = process.argv[2] || 'Test_Template/lab_digital.pcbsim';
...
for (let i = 0; i < doc.tracks.length; i++) {
  for (let j = i + 1; j < doc.tracks.length; j++) {
    ...
    if (d + 1e-6 < need) {
      clash++;
      if (details.length < 40) details.push('C ' + t1.netName + '/' + t2.netName + ' ' + t1.layer + ' d=' + d.toFixed(1));
    }
  }
}
...
for (const fp of fps.filter(f => /^U/.test(f.refDes))) {
  const pads = fp.pads.filter(p => p.netName).map(p => p.number + ':' + p.netName).join(' ');
  console.log(fp.refDes, pads);
}
