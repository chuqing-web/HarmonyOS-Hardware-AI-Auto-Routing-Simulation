...
3. 导线布局（硬门禁）:
   - 导线不得进入任何器件的「选中命中区」(HIT_PAD=14，与编辑器选中范围一致)
   - 导线不得贴近/碰到无关引脚（安全距 ≥20mil）；仅允许连接本网引脚
   - 不同网络导线不得共线重叠
   - 审查时必须对照「器件选中区AABB」与「导线完整路径覆盖」；违规 → fixAction=reroute 或改用 joinByLabel
...
8. LED: 每颗 LED 是否已有串联限流电阻(R_330 等)? 不要在已有足够电阻时再加电阻
9. 【互斥双色开关】用户要「打开绿灯/闭合红灯」时:
...
10. 【完整生图门禁】必须消除全部阻断项：
   ...
   - 几何 error：wire_body(穿选中区) / pin_proximity(碰无关脚) / wire_cross(异网正交交叉或共线重叠)
